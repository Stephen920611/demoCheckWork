...
import styles from './AddRetinueRecordDetail.less';
...
@connect(({addInfo, checkRecord, loading}) => ({
    addInfo,
    checkRecord,
    fetchEditStatus: loading.effects['checkRecord/fetchDetailInfoAction'],
}))
@Form.create()
class AddRetinueRecordDetail extends PureComponent {
    constructor() {
        super();
        this.state = {
            formItemLayout: {
            ...
            new Promise((resolve, reject) => {
                ...
                if (response.code === 0) {
                    self.props.form.setFieldsValue({
                        ...
                        dateArrive: T.lodash.isUndefined(data.dateArrive) ? '' :  data.dateArrive === null ?  '' : T.moment(new Date(data.dateArrive).getTime()),
                        dateConvertConfirm: T.lodash.isUndefined(data.dateConvertConfirm) ? '' : data.dateConvertConfirm === null ?  '' : T.moment(new Date(data.dateConvertConfirm).getTime()),
                        dateConvertSuspect: T.lodash.isUndefined(data.dateConvertSuspect) ? '' : data.dateConvertSuspect === null ?  '' : T.moment(new Date(data.dateConvertSuspect).getTime()),
                        dateLeftPass: T.lodash.isUndefined(data.dateLeftPass) ? '' :data.dateLeftPass === null ?  '' : T.moment(new Date(data.dateLeftPass).getTime()),
                        dateRelease: T.lodash.isUndefined(data.dateRelease) ? '' : data.dateRelease === null ?  '' : T.moment(new Date(data.dateRelease).getTime()),
                        ...
                        touchDate: T.lodash.isUndefined(data.touchDate) ? '' : data.touchDate === null ?  '' : T.moment(new Date(data.touchDate).getTime()),
    ...
    onSubmitData = (e) => {
        ...
        form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                ...
                new Promise((resolve, reject) => {
                    dispatch({
                        type:infoEdit ? 'checkRecord/updateInfoAction' :'checkRecord/addInfoAction',
                        params,
                        resolve,
                        reject,
                    });
...
export default AddRetinueRecordDetail;
